...
class PipelineFactory(IPipelineFactory):
    ...
    async def create_pipeline_async(self, input_port: TInputPort, pipeline_configuration: List[Type[IPipe]]) -> List[Type[IPipe]]:
        ...
        _PipeClasses = [Common.import_class_by_namespace(_Namespace) for _Namespace in self._usecase_registry[_UsecaseKey]]

        _PipeServices = [self._service_provider.get_service(_PipeClass) for _PipeClass in _PipeClasses]

        _FilteredPipes = [_Pipe for _Pipe in _PipeServices if any(issubclass(_Pipe.__class__, _PipeType) for _PipeType in pipeline_configuration)]

        _SortedPipes = sorted(_FilteredPipes, key=lambda _Pipe: pipeline_configuration.index(next(_PipeType for _PipeType in pipeline_configuration if issubclass(_Pipe.__class__, _PipeType))))

        _AdditionalPipes = [_ExtraPipe for _ExtraPipe in pipeline_configuration if not any(issubclass(_Pipe.__class__, _ExtraPipe) for _Pipe in _SortedPipes)]

        for _Pipe in _AdditionalPipes:
            _SortedPipes.insert(pipeline_configuration.index(_Pipe), _Pipe())
